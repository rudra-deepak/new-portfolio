...
const ProjectCard = ({ project, aosDelay }) => (
  <div 
    ...
    className={`relative rounded-2xl p-[1px] group transition-all duration-500 ${
      project.isFlagship 
        ? 'bg-gradient-to-br from-yellow-500/50 via-white/10 to-yellow-500/30 hover:from-yellow-500 hover:via-yellow-400/30 hover:to-yellow-500/60' 
  ...
  >
    <div className={`rounded-2xl p-6 md:p-8 h-full backdrop-blur-md transition-all duration-500 ${
      ...
      {/* Badge */}
      {project.badge && (
        <span className="inline-block text-xs font-bold tracking-widest uppercase text-yellow-400 bg-yellow-500/10 px-3 py-1 rounded-full border border-yellow-500/20 mb-4">
          {project.badge}
        </span>
      )}
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {project.techTags.map((tag) => (
          <span 
            key={tag}
            className="px-3 py-1 text-xs font-bold text-white/70 bg-white/5 rounded-full border border-white/10 hover:bg-yellow-500/20 hover:border-yellow-500/30 hover:text-yellow-300 transition-all duration-300 cursor-default"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {project.links.githubs && project.links.githubs.map((gh) => (
  <a 
    key={gh.url}
    href={gh.url}
    target="_blank"
    ...
    {gh.label}
        ...
        {project.links.demo !== undefined && (
          <a 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-300 ${
              project.links.demo 
                ? 'bg-[#36013F] text-white hover:bg-yellow-600 hover:shadow-[0_0_20px_rgba(255,42,42,0.4)]' 
        ...
        {project.links.frontendDemo && (
          <a 
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#36013F] text-white text-sm font-semibold hover:bg-yellow-600 hover:shadow-[0_0_20px_rgba(255,42,42,0.4)] transition-all duration-300"
          >
            <ExternalLinkIcon />
            Frontend Demo
          </a>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div data-aos="fade-up" className="mb-16 md:mb-20">
          ...
          <p className="text-white/50 text-base md:text-lg max-w-lg font-medium leading-relaxed">
            A selection of projects that showcase my expertise in full-stack development and modern architecture in AI.
          </p>
        </div>
